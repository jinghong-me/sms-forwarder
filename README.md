...
轻量、开源的 Android 应用，用于将接收到的短信转发到企业微信/钉钉/飞书/通用 Webhook。设计目标是：小、稳、兼容性好，支持在厂商定制系统（如小米澎湃OS/HyperOS）上长期驻留运行并尽量减少被系统杀死的概率。
...
短信转发助手3.0版本（飞书支持、去重、重试、持久化） [https://github.com/lanbing1989/sms-forwarder/releases](https://github.com/lanbing1989/sms-forwarder/releases)
...
### v3.0 更新（最新）
在 v2.0 的基础上，进一步提升可靠性与用户体验：

- 新增飞书（Feishu）通道支持：现在支持企业微信、钉钉、飞书、通用 Webhook 四种通道类型。
- 消息去重机制：5秒内相同发送者和内容的消息会被自动去重，避免重复转发。
- 失败消息重试：转发失败的消息最多重试3次，采用指数退避策略（2s、4s、6s间隔）。
- 失败消息持久化：转发失败的消息会保存到本地文件（最多100条），应用重启或网络恢复时自动重试。
- 网络监听：监听网络状态变化，网络恢复时自动触发失败消息重试。
- 规则测试功能：在 App 内可测试当前配置的关键词规则是否匹配预期。
- 电池优化引导：引导用户将应用加入电池优化白名单，提高服务存活率。
- 关于对话框：添加版本信息和应用介绍。

### v2.0 更新
在保留第一版简单易用特性的同时，本次更新增加/改进了若干点以提升可用性与扩展性：

- 多通道与规则：支持多个转发通道（企业微信/钉钉/通用 Webhook），以及把关键词映射到不同通道的规则（空关键词表示转发全部）。
...
- 支持企业微信、钉钉、飞书、任意 HTTP(S) Webhook 四种转发通道。
- 消息去重：5秒内相同发送者和内容的消息自动去重，避免重复转发。
- 失败重试：转发失败的消息最多重试3次，采用指数退避策略。
- 失败持久化：转发失败的消息保存到本地文件（最多100条），应用重启或网络恢复时自动重试。
- 网络监听：监听网络状态变化，网络恢复时自动触发失败消息重试。
- 规则测试：在 App 内可测试当前配置的关键词规则是否匹配预期。
- 电池优化引导：引导用户将应用加入电池优化白名单，提高服务存活率。
...
- 并行与可靠性：
  - webhook 请求采用 OkHttp；每次请求最多重试 3 次，采用指数退避（2s、4s、6s 间隔）。
  - SmsReceiver 使用 goAsync + 线程池并在所有任务完成后调用 pendingResult.finish()。
  - 消息去重：5秒内相同发送者和内容的消息会被自动去重。
  - 失败消息持久化：失败消息保存到本地文件（最多 100 条），应用重启或网络恢复时自动重试。
- UI 注意：
...
- v2.0 — 重构与增强：多通道、多关键词、Compose UI、深色模式与沉浸式状态栏适配、日志与运行时兼容性优化。
- v3.0 — 可靠性与用户体验升级：飞书通道支持、消息去重、失败重试与持久化、网络监听、规则测试、电池优化引导、关于对话框、统一消息前缀格式。
